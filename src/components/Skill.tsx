    ...
    <div className="skill text-white p-4 text-center ">
      ...
      <div className="flex flex-wrap justify-center gap-3">
        {icons.map((data, idx) => (
          <div
            key={idx}
            // Tambahkan `tabindex="0"` agar div bisa di-focus
            // Tambahkan `group-focus-within:` pada kelasnya
            className="relative flex items-center justify-center bg-red-800/50 rounded-full w-12 h-12 border border-red-500 shadow-lg shadow-red-600/50 hover:scale-105 hover:-translate-y-2 focus-within:scale-105 focus-within:-translate-y-2 transition duration-300 ease-in-out group"
            tabIndex={0} // Membuat div ini dapat di-focus
          >
            <img
              src={data.icon}
              alt={data.label}
            ...
            />
            <span
              className={`absolute ${data.position} 
              group-hover:animate-bounce-custom 
              group-focus-within:animate-bounce-custom 
              border border-red-500 shadow-md shadow-red-600/50 
              bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 rounded 
              bg-red-800/50 text-[10px] text-white opacity-0 
